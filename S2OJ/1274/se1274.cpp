...
bool chq(int step)
{
	...
	to[0] = temp[0], to[1] = temp[1], to[2] = temp[2];
	up(ori[0], ori[1], ori[2], step);
	if((to[0] != temp[0]) || (to[1] != temp[1]) || (to[2] != temp[2]))return false;
	return true;
}
int main()
{
	...
	if((to[0] != temp[0]) || (to[1] != temp[1]) || (to[2] != temp[2]))
	{
		puts("NO");
		return 0;
	}
	if(step1 < step2)
	{
		...
		goal[0] = temp[0], goal[1] = temp[1], goal[2] = temp[2];
	}
	else if(step1 > step2)
	{
		ans += step1 - step2;
		up(ori[0], ori[1], ori[2], step1 - step2);
		ori[0] = temp[0], ori[1] = temp[1], ori[2] = temp[2];
	}
